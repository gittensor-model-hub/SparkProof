#!/usr/bin/env bash
# Doc-grounded dataset → teacher → prove → verify → SFT → [train Qwen].
#
#   scripts/run_doc_qwen.sh --limit 2
#   scripts/run_doc_qwen.sh --run-id doc-qwen-001 --train
set -euo pipefail
...
usage() {
  cat <<'EOF'
usage: scripts/run_doc_qwen.sh [options]

Doc seeds (api + semantics + tutorials) → sparkproof-triton-generate → verify → SFT → [train]

Options:
  --run-id ID              default: doc-qwen-YYYYMMDD-HHMMSS
  --prompts PATH           default: prompts/<run-id>.jsonl
  --bundle PATH            default: bundles/<run-id>
  --limit N                cap seeds (smoke tests)
  --source SOURCE          only this prompt source (repeatable)
  --task-id ID             only this task_id (repeatable)
...
run_id="${run_id:-doc-qwen-$(date +%Y%m%d-%H%M%S)}"
prompts="${prompts:-prompts/${run_id}.jsonl}"
bundle="${bundle:-bundles/$run_id}"
sft_out="$SD/data/processed/${run_id}_sft.jsonl"

echo ">>> building doc prompts (api + semantics + tutorials)"
build_args=(--out "$prompts" --sources api_doc,doc_semantics,doc_tutorial)
...
echo "doc → Qwen pipeline complete"
