...
def test_openrouter_ledger_accepts_dated_fable_build_suffix():
    def fetch(generation_id, api_key):
        return {"data": {"id": generation_id, "model": "anthropic/claude-fable-5-20260709"}}

    record = _openrouter_record(model="anthropic/claude-fable-5-20260709")
    issues = verify_openrouter_generations([record], api_key="k", fetch=fetch)
    assert issues == []


def test_openrouter_ledger_accepts_legacy_fable_record_with_dated_ledger():
    def fetch(generation_id, api_key):
        return {"data": {"id": generation_id, "model": "anthropic/claude-fable-5-20260709"}}

    record = _openrouter_record(model="anthropic/claude-fable-5")
    issues = verify_openrouter_generations([record], api_key="k", fetch=fetch)
    assert issues == []
